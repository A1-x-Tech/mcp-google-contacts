...
/**
 * Missing credentials must not throw: the server starts degraded and the
 * client raises CredentialsError on the first call instead (pinned in
 * client.test.ts). Reverting this would kill the process before the MCP
 * handshake and leave the user with a dead server and no reason.
 */
test("no credentials at all is not an error — the config loads with empty fields", () => {
  withEnv({}, () => {
    ...
    assert.equal(config.apiBase, "https://people.googleapis.com");
    assert.equal(hasCredentials(config), false);
  });
});

test("a partial OAuth triple reports incomplete_oauth_config", () => {
  assert.equal(reasonOf({ GOOGLE_CONTACTS_CLIENT_ID: "id" }), "incomplete_oauth_config");
  assert.equal(
    reasonOf({ GOOGLE_CONTACTS_CLIENT_ID: "id", GOOGLE_CONTACTS_CLIENT_SECRET: "secret" }),
  ...
  assert.equal(
    reasonOf({ GOOGLE_CONTACTS_REFRESH_TOKEN: "rt", GOOGLE_CONTACTS_ACCESS_TOKEN: "at" }),
    "incomplete_oauth_config",
  );
});
  ...
  withEnv(
    {
      ...
      GOOGLE_CONTACTS_REFRESH_TOKEN: "rt",
    },
    () => {
      const config = loadConfig();
...
test("a static access token alone is enough", () => {
  withEnv({ GOOGLE_CONTACTS_ACCESS_TOKEN: "at" }, () => {
    const config = loadConfig();
...
test("invalid numeric overrides fall back to the defaults", () => {
  withEnv(
    {
      GOOGLE_CONTACTS_ACCESS_TOKEN: "at",
      GOOGLE_CONTACTS_TIMEOUT_MS: "not-a-number",
      GOOGLE_CONTACTS_MAX_RETRIES: "-5",
    },
    () => {
      const config = loadConfig();
...
test("numeric overrides are honored when valid", () => {
  withEnv(
    { GOOGLE_CONTACTS_ACCESS_TOKEN: "at", GOOGLE_CONTACTS_TIMEOUT_MS: "1000", GOOGLE_CONTACTS_MAX_RETRIES: "0" },
    () => {
      const config = loadConfig();
