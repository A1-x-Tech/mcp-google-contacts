...
before(() => {
  ...
  // GOOGLE_CONTACTS_* live and re-reads $XDG_CONFIG_HOME files per call.
  for (const key of [...ENV_KEYS, "XDG_CONFIG_HOME"]) savedEnv.set(key, process.env[key]);
  for (const key of ENV_KEYS) delete process.env[key];
  configDir = mkdtempSync(join(tmpdir(), "mcp-contacts-auth-test-"));
...
test("options pin this server's wiring: env prefix, server name and documented scopes", () => {
  assert.equal(AUTH_OPTIONS.envPrefix, "GOOGLE_CONTACTS", "must read the same variables as config.ts");
  assert.equal(AUTH_OPTIONS.serverName, "contacts", "token path must be mcp-google-contacts/credentials.json");
...
test("env priority flows through the provider: GOOGLE_CONTACTS_ACCESS_TOKEN wins (invariant 3)", async () => {
  const { server } = fakeServer();
  const provider = registerAuthTools(server);
  process.env.GOOGLE_CONTACTS_ACCESS_TOKEN = "ENV-TOK";
  try {
    ...
    delete process.env.GOOGLE_CONTACTS_ACCESS_TOKEN;
  }
...
test("auth_status routes to the provider: not connected, this server's paths, no error", async () => {
  ...
  assert.match(text, /mcp-google-contacts/, "must report this server's own credentials path");
});
...
test("the unconfigured prefix offers the in-chat login first and the env path as alternative", () => {
  ...
  assert.match(prefix, /GOOGLE_CONTACTS_CLIENT_ID/);
  assert.ok(
    prefix.indexOf("start_login") < prefix.indexOf("GOOGLE_CONTACTS_CLIENT_ID"),
