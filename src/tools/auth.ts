...
/**
 * `verifyIdentity` for the component: one cheap Google Contacts read, plus the
 ...
 * is the point: OIDC answers even when Google People API is switched off, so a
...
async function verifyIdentity(accessToken: string): Promise<{ email?: string }> {
  try {
    ...
    if (error instanceof GoogleContactsError && error.status === 403 && isApiDisabled(error.body)) {
      throw new OAuthError("accessNotConfigured", "Google People API", error.status);
    }
    throw error;
  }
...
/**
 * The single source of the auth wiring: serverName "contacts" puts the stored
 * login in ~/.config/mcp-google-contacts/credentials.json (the component
 * prefixes "mcp-google-"), envPrefix keeps the provider reading the exact
 * GOOGLE_CONTACTS_* variables config.ts documents — which is what preserves the
