...
test("raw_request defaults to GET with the Bearer token", async () => {
  ...
  try {
    const res = await tools.raw_request({ path: "v1/people/c1?personFields=names" });
    assert.equal(res.isError, undefined);
    assert.equal(calls[0].method, "GET");
    assert.equal(calls[0].url, "https://people.googleapis.com/v1/people/c1?personFields=names");
...
test("raw_request PATCHes a JSON body to a relative path", async () => {
  ...
  try {
    await tools.raw_request({
      path: "v1/people/c1:updateContactPhoto",
      method: "PATCH",
      body: { photoBytes: "QUJD" },
    });
    assert.equal(calls[0].method, "PATCH");
    assert.equal(calls[0].url, "https://people.googleapis.com/v1/people/c1:updateContactPhoto");
    assert.deepEqual(calls[0].body, { photoBytes: "QUJD" });
  } finally {
    restore();
  }
});
