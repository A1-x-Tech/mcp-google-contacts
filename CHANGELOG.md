...
- Tokens from a login are stored per server in
  `~/.config/mcp-google-contacts/credentials.json` (0600) and re-read on every
  call, so a login finished mid-session works without restarting the AI client.
  `GOOGLE_CONTACTS_OAUTH_PORT` pins the loopback listener port for SSH forwarding.
- `finish_login` verifies a fresh login against **Google People API** itself rather than
...
- The client accepts the component's `TokenProvider` as a fallback token
  source: environment credentials (the refresh triple or `GOOGLE_CONTACTS_ACCESS_TOKEN`)
